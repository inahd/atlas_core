...
This build is designed as a _relational, citation-forward knowledge layer_ that can power an NPU-style knowledge graph (KG) while explicitly separating _classical attestation_ from _modern interpretation_ and _speculative correspondences_.

=== Core sources used in this research package


The most “machine-ready” herb-property source identified is the _Amidha Ayurveda Herb Database (Beta)_: an open dataset (CC BY 4.0) with 700+ entries including _rasa, guṇa, vīrya, vipāka, prabhāva,_ and _doṣa karma_; its release notes also claim verification against classical and pharmacopeial sources (Charaka, Suśruta, Bhāvaprakāśa Nighaṇṭu, API). citeturn28view0turn5view0

For _official botanical identity + parts used_ (high value for KG disambiguation), the Ministry of AYUSH/NMPB _e-Charak Knowledge Resources_ page provides a usable tabular list mapping botanical names to trade names and parts used (including key priority items like _Śilājatu_, _Guggulu_, _Aśvagandhā_, _Āmalakī/Amla_, _Brahmī_, _Śaṅkhapuṣpī_, _Bālā_, etc.). citeturn23view0

For _graha ↔ plant_ and _nakṣatra ↔ plant_ correspondences with comparatively stronger institutional provenance, an ICAR page documents “Navagraha Vatikā” and “Nakṣatra Vatikā” plant lists (including Surya→Arka/Calotropis, Chandra→Palāśa/Butea, etc., and Bharanī→Āmla, Kṛttikā→Udumbara). citeturn15view0

For mapping _nakṣatra ↔ doṣa_, the most explicit structured table found is via the _Nāḍī Kūṭa_ tradition used in Aṣṭakūṭa matchmaking: Adi/Madhya/Antya Nāḍī correspond to Vāta/Pitta/Kapha, with a published mapping of all 27 nakṣatras. This is _Jyotiṣa tradition_, not a standard classical Āyurveda table, so it should be treated as “OBSERVED (Jyotiṣa practice)” rather than “OBSERVED (Caraka/Suśruta).” citeturn18view0

For Pañcāṅga time primitives, _tithi_ is defined as a lunar day based on Sun–Moon angular separation and there are 30 tithis in a lunar month. citeturn20search1
A _muhūrta_ is widely defined as _1/30 of a day ≈ 48 minutes_ in Brāhmaṇa-era time divisions (standard secondary references summarize this unit). citeturn20search5
Ekādaśī is operationally a fasting observance tied to the 11th tithi; Drik Panchang documents Ekādaśī fasting timing concepts such as _parāṇa_ windows. citeturn20search0

=== Attestation model you can encode directly in the KG


Use _two separate notions_ rather than one:
...
- _Standard Dravyaguṇa_: `rasa, virya, vipaka, prabhava, dosha_effect` (strong fit; widely described as key drug properties in Āyurveda pharmacology literature and classical framing). citeturn9search11turn28view0
- _Clinical/physiology linking_: `dhatu_affinity, srota_affinity` (plausible, but often interpretive unless you cite a specific lexicon/teacher tradition)
- _Astro-correspondence_: `nakshatra_correspondence, graha_correspondence, panchanga_timing` (typically _not_ standardized in Āyurveda classics; best practice is to (a) bind to institutional lists where available, and (b) otherwise label as speculative/inferential). citeturn15view0turn18view0

The most robust way to implement this in a KG is to _store these as separate relationships with provenance_, not as single-string columns. Example:
...
The records below show _how to populate “cross-domain” columns safely_: when an institutional list exists (ICAR), it is tagged `OBSERVED:JYOTISHA_TRADITION`; when inferred, it is `SPECULATIVE:INFERENCE_RULE`. citeturn15view0turn28view0
...
- The _ICAR Nakṣatra Vatikā_ explicitly links _Bharanī ↔ Āmla_ (and many other nakṣatras ↔ trees/plants), making that kind of nakṣatra correspondence materially stronger than generic “Ayurveda–astrology” blog mappings. citeturn15view0
- The _ICAR Navagraha Vatikā_ gives explicit graha↔plant links (e.g., Surya→Rui/Calotropis, Chandra→Palāśa/Butea, Budha→Apāmārga/Achyranthes, etc.). Use these as _OBSERVED:JYOTISHA_TRADITION_ correspondences rather than inventing new ones. citeturn15view0
...
For Pañcāṅga integration, the KG should treat _time_ as a _computed context_: current tithi/nakṣatra/vara are inputs, and recommendations are outputs.

Because you requested “8 muhurtas to midnight,” the table below uses _eight broad time blocks_ (not the full 30×48-minute muhūrta grid). The 48-minute technical definition of muhūrta is still important for precision anchoring. citeturn20search5
...
Ekādaśī is explicitly treated as a _fasting tithi_ with timing/observance conventions; Drik Panchang provides operational guidance such as timings and fasting-related concepts, which is what you need for computable KG rules. citeturn20search0

=== Vara modifiers layer


A KG-friendly pattern is to store _vara modifiers_ as a separate table, so daily routines can be computed as:
...
A complete 27-row matrix is possible using the Nāḍī Kūṭa mapping of nakṣatras into _Adi (Vāta)_, _Madhya (Pitta)_, _Antya (Kapha)_. citeturn18view0
This gives a _clean, computable_ primary-doṣa signal. Secondary-doṣa and “modification_when_active” are best treated as _inference rules_, unless you adopt a single named Jyotiṣa-Ayurveda lineage with its own explicit attributions.
...
Even if you later revise details, the KG benefits from a stable _dhātu ontology_ with:

- IDs and canonical names
- doṣa affinity tendencies
- nourishment/support edges: foods, herbs, practices
- seasonal peak as a _probabilistic modifier_ (not hard rule)

Because your request specifies “governing graha (from BPHS),” but BPHS-specific citations were not fully extracted in this session, the graha field below is provided as _SPECULATIVE_ and should be replaced with an explicitly cited BPHS edition mapping once you select an edition and extract the relevant verses.
...
Below is a KG-friendly _rule layer_ where each rule has:
...
Rules referencing _Ekādaśī_ are the most operationally grounded in Pañcāṅga practice because the tithi is well-defined and commonly used for fasting. citeturn20search0turn20search1
Rules referencing _nakṣatra doṣa_ depend on the Nāḍī Kūṭa mapping (Jyotiṣa tradition). citeturn18view0
Rules referencing _graha↔plant_ can be anchored in the ICAR Navagraha Vatikā list for those specific plants. citeturn15view0
...
- It keeps _Ayurveda pharmacology_ (Dravyaguṇa properties) separated from _Jyotiṣa_ constructs (nakṣatra doṣa via Nāḍī, graha plants via Navagraha Vatikā), yet allows them to meet at a computable decision layer. citeturn28view0turn18view0turn15view0
- It treats _Pañcāṅga time_ (tithi/muhūrta) as a _context engine_ rather than a static attribute. citeturn20search1turn20search5
...
- A _curated 108-herb index spine_ (IDs + canonical names) ready for KG ingestion.
- Fully specified _CSV schemas_ for Dinācaryā×Pañcāṅga, Ṛtucaryā, Nakṣatra×Doṣa, Sapta Dhātu, and Coherence Rules.
- A defensible provenance strategy and explicit attestation labeling.

To produce the _fully populated 108×(full Dravyaguṇa + dhātu/srota + nakṣatra/graha + timing)_ master table at high fidelity, the recommended build path is:

- Join the 108-herb spine to the _Amidha dataset_ for `rasa/guna/virya/vipaka/prabhava/dosha` (OBSERVED:MODERN_SECONDARY). citeturn28view0
- Join to _AYUSH e-Charak_ for botanical identity + parts used where present (OBSERVED:AYUSH_GOV). citeturn23view0
- Join to _ICAR Navagraha/Nakshatra Vatikā_ for _only those plants explicitly listed_ (OBSERVED:JYOTISHA_TRADITION). citeturn15view0
- For remaining cross-correspondences, keep them _blank by default_ until you adopt a single explicit lineage/source—otherwise they should be generated as _SPECULATIVE inference edges_ (never as “facts”).
