...
#compendium-preamble(
  title: "Two-Source Interference on the Zodiacal Circle",
...
#title-page(
  title: "Two-Source Interference on the Zodiacal Circle",
  subtitle: "Aspect theory as harmonic selection",
...
_Converted from `research/two-source-interference-v3.md`. Content-lossless conversion._
...
_RESEARCH-017_ | April 16, 2026 | Extension of yantra eigenvalue exploration
...
_Distinction from earlier Chladni/Sri Yantra work._ The N-source Chladni figures
studied previously concern the 2D nodal geometry _inside_ the unit disk — radial
Bessel-function nodes producing mandala-like patterns in the disk interior. This
section concerns amplitude _on the circular boundary_ — the 1D zodiacal ring.
...
_Claim._ N equally-spaced sources at wavenumber k on the unit circle produce:

- A(θ) = 0 identically when N does not divide k
- A(θ) = N·cos(kθ + φ) when N divides k (standing wave at the k-th harmonic)

The boundary field is not merely "flat" at non-resonant k — it is _zero_.
Each N-fold division has a single self-resonance harmonic and is silent below it.

_Setup._ Place N sources at θ_n = 2πn/N, n = 0, ..., N-1. Each radiates
...
_Proof._ Expand using Re[e^{ix}] = cos(x):
...
_Consequence for classical divisions._ For jyotish-relevant harmonics k = 1
through 12:
...
_Claim._ Two point sources on the zodiacal circle, each radiating cos(k · d),
...
_Setup._ Place two sources at sidereal longitudes α and β. The amplitude
...
_The envelope cos(k(α-β)/2) = ±1_ when k(α-β)/2 = nπ, i.e., when the
angular separation (α-β) = n · 360°/k. At these separations, the two sources
are in perfect constructive interference.

_Classical aspect correspondence._ The constructive maxima at separation
360°/k are:
...
_Claim._ When two sources are exactly 180° apart (as Rahu and Ketu always are),
the interference pattern depends on the parity of k:

- _Odd k:_ A(θ) = 0 identically. The field vanishes everywhere.
- _Even k:_ A(θ) = 2·cos(kθ). A rigid angular grid with 2k equally-spaced nodes.

_Proof._ Let α = 0, β = π (WLOG, by rotation). Then:
...
_Structural consequence._ The Rahu-Ketu axis is invisible at odd harmonics
...
_Setup._ The 27×30 wave matrix W is computed by evaluating two-source Sun-Moon
...
_Claim (gross level — negative result)._ The dominant structure of W is a rank-1
...
_Claim (fine structure — positive result)._ After rank-1 subtraction, the
...
_Connection to Finding 9._ The even-harmonic dominance of the tithi-group signal
...
_Dimensional observation._ The wave matrix W has effective rank 15 (number of
...
_Claim._ The 6 gandanta nakshatras (fire-water sign junctions: Ashwini,
...
_Evidence._ Two-sample t-test on differentiation scores: gandanta mean = 1.72,
non-gandanta mean = 1.08, t = 9.727, p < 0.0001.

_Pattern within gandanta._
...
_Same-lord similarity._ Nakshatras sharing the same nakshatra lord show weak
...
_Correction on mirror symmetry._ Apparent mirror symmetry between zodiac-opposite
...
_Claim._ The classical Panchaka system and the wave interference system describe
orthogonal structures on the 27×30 nakshatra-tithi space.

_The Panchaka matrix._ Panchaka is computed as (nakshatra_index + tithi_index) mod 9,
...
_The wave matrix._ The Sun-Moon two-source interference matrix has effective
rank 15 after fine-structure extraction.

_Orthogonality._ Pearson correlation between the two matrices: r = -0.011.
...
_Source._ GFZ Helmholtz Centre, Potsdam (Matzka et al., 2021). 34,439 days
...
_Gandanta variability._ Gandanta nakshatras showed elevated Kp variability in
...
_Cell-level analysis._ 3 cells survive Benjamini-Hochberg FDR correction at
...
_Conclusion._ The wave interference model does not predict geomagnetic
...
_Setup._ Combined Sun-Moon tidal potential computed at each nakshatra midpoint
...
_Gandanta-restricted analysis (partial positive)._
...
_Interpretation._ At the zodiacal points where the wave model predicts maximal
...
_Computed from Swiss Ephemeris, geocentric positions, 2000–2030._
...
_Connection to the Chladni resonance disk._ The N=11 base of the Chladni
resonance computation (from the Sri Yantra eigenvalue paper) matches Jupiter's
retrograde station count exactly. The Chladni disk at N=11 _is_ Jupiter's
orbital geometry projected onto a vibrating membrane.

=== Part B: Mode Correspondence


_Claim._ The angular modes injected by the two-source wave interference
...
_Mode injection by source._ The Sun-Moon tithi cycle (Finding 10) injects
...
the wave field _only_ through Jupiter's aspect structure. Venus itself,
having no special aspects beyond the universal 7th, does not inject its own
retrograde symmetry; Jupiter does it for her.

_Composite emergent amplification._ When all two-source pairs are computed
...
_Computed with no free parameters except radial window width._
...
_44 distinct enclosed regions_ (22 positive, 22 negative).

_Comparison._ The Sri Yantra has 43 sub-triangles in the standard enumeration.
Some traditions count 43 + bindu (the central point) = 44.

_Other configurations tested:_
...
| _Tidal-weighted (M/a³), overlapping_ | _44_ |
...
_Preliminary — requires full computation to verify._
...
_(a) Atmospheric tidal testing._ The gandanta-tidal convergence (Finding 13)
...
_(b) Precipitation records._ Rainfall data (India Meteorological Department,
100+ year records) could test the Varāhamihira-era claims in Bṛhat Saṃhitā
about nakshatra-dependent precipitation patterns.

_(c) Quiet-time lunar geomagnetic variation._ The L2 current system, distinct
...
_(d) Multi-chart evaluation._ Wave field composite activation scores should be
...
_(e) Nitya dimensionality._ The rank-15 dimensionality of the wave matrix and
...
_(f) k-weighting optimization._ Classical aspect strengths (7th = full,
...
_(g) Quantized field topology._ The 44-region tidal-weighted field should be
...
_(h) Tithi-modulated region count._ Compute the full tithi-modulated quantized
...
_(i) Venus-Shakti correspondence._ Venus's pentagram (5-fold retrograde trace)
