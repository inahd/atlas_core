...
#compendium-preamble(
  title: "Yantra Eigenvalue Exploration",
...
#title-page(
  title: "Yantra Eigenvalue Exploration",
  subtitle: "Lo Shu Kronecker products and the Brahmasthana Theorem",
...
_Converted from `research/yantra_eigenvalue_exploration.md`. Content-lossless conversion._
...
_RESEARCH-016_ | April 12, 2026 | Pratah-lila | Guru hora
...
_Magic constant M = 27_ | Eigenvalues: {27, +4.899, -4.899}
...
| lambda_2 | +2sqrt(6) = +4.8990 | _No_ (invariant) |
| lambda_3 | -2sqrt(6) = -4.8990 | _No_ (invariant) |

The +/-2sqrt(6) is a _universal constant of the Lo Shu_. It does not depend
on which graha is active. It is the irreducible spatial asymmetry of the
Vastu Purusha Mandala.

_Proof_: Adding k*J to any matrix shifts only the eigenvalue corresponding
...
_Interpretation_: As k increases, M grows while +/-2sqrt(6) stays fixed.
...
_Scaling law_: M_n = M_1^n (magic constant raised to the level power).
...
_Magic property preserved at ALL levels_: row sums = column sums = M^n.
...
but a _geometric principle_: the state of maximum coherence where all
...
_Eigenvalues_: {81, +/-14.697, 0, 0, 0, 0, 0, 0}

Where 14.697 = 3 × 2√6. The factor of 3 comes from the Lo Shu structure
of the outer placement.

_Rank = 5_ (not 9). Four zero eigenvalues mean four degrees of freedom
have collapsed — the composite is more constrained than a general 9x9 magic square.

The block magic constants form **3_Lo_Shu + 12_J** — the meta-structure
is itself a magic-square pattern. The navagraha composite is a
_magic square of magic squares_.
...
_lambda_1 = 27 (Brahmasthana)_: [-0.577, -0.577, -0.577] — uniform.
All zones equal. Maximum coherence.

_lambda_2 = +4.899 (primary tension)_: [-0.075, -0.667, +0.742]
Axis: South row (SE/Agni, S/Yama, SW/Nirrti) vs center row.
The field's dominant spatial polarity.

_lambda_3 = -4.899 (secondary tension)_: [-0.742, +0.667, +0.075]
...
This is not random variation — it is the _breathing of the Vastu field_.
Sun hora = maximum spatial differentiation = most dynamic field.
Saturn hora = maximum spatial uniformity = most stable field.

The 2sqrt(6) invariant is the _heartbeat_ that never changes.
