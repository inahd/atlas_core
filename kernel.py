...
def create_app():
    ...
    # OSC client for SuperCollider communication
    _osc_client = None
    try:
    ...
    def _spine():
        ...
        try:
            ...
            try:
                ...
                payload["sound_spec"] = derive_sound_spec(_sound_fs, mode=_sound_mode)
            except Exception:
                payload["sound_spec"] = {}
    ...
    @app.route("/render/toroid", methods=["POST", "GET"])
    def _render_toroid():
        ...
        try:
            result = subprocess.run(
                ["blender", "--background", "--python",
                 str(Path(_here) / "blender" / "render_toroid.py")],
                capture_output=True, text=True, timeout=60)
            if Path("/tmp/atlas_render.png").exists():
                return send_file("/tmp/atlas_render.png", mimetype="image/png")
            return app.response_class(json.dumps({"error": result.stderr[-300:]}), mimetype="application/json", status=500)
        except subprocess.TimeoutExpired:
            return app.response_class(json.dumps({"error": "render timeout"}), mimetype="application/json", status=504)
        except FileNotFoundError:
            return app.response_class(json.dumps({"error": "blender not installed"}), mimetype="application/json", status=503)
    ...
    @app.route("/render/eternal")
    def _render_eternal():
        """Eternal cosmology render — canonical entity positions, not field-weighted.

        Uses all loaded entities with their canonical theta/phi coordinates.
        The t parameter can advance through ashtakala phases.
        """
        ...
        try:
            ...
            # Build canonical entity list from datasets
            ...
            for eid, (theta, phi) in list(coords.items())[:64]:
                ...
                entities.append({
                    ...
                    "theta": theta,
                    "phi": phi,
                    "composite_score": 0.5,  # uniform — eternal, not field-weighted
    ...
    # ── Async bija playback through server speakers ──
    ...
    @app.route("/bija/<bija_id>/play", methods=["POST", "GET"])
    ...
    @app.route("/bija/<bija_id>/synthesize", methods=["POST", "GET"])
    def _bija_synthesize(bija_id):
        ...
        if mode == "play":
            # Play through server speakers, return JSON
            dur = float(request.args.get("duration", 3.0))
            _play_bija_async(bija_id, dur)
            return app.response_class(
        ...
        try:
            ...
            with wave.open(buf, 'wb') as wf:
                wf.setnchannels(1)
                wf.setsampwidth(2)
                wf.setframerate(48000)  # MOTU M2 native rate
    ...
    @app.route("/sound/volume", methods=["GET", "POST"])
    def _sound_volume():
    ...
    # ── SOUND CONTROL ENDPOINTS (OSC → SuperCollider) ──────
    ...
    def _sound_play_note():
        ...
        sa = 261.63 * (2 ** octave)
        freq = sa * (2 ** (semi / 12))
        _send_osc("/melody", [freq, dur, vel, 0.0])
        return app.response_class(
            json.dumps({"freq": freq, "sent": True}, ensure_ascii=False),
            mimetype="application/json",
        )

    @app.route("/sound/voice", methods=["POST"])
    def _sound_voice():
        data = request.get_json(force=True)
        vtype = data.get("type", "konnakol")
        syllable = data.get("syllable", "ta")
        if vtype == "konnakol":
            _send_osc("/voice", [syllable.lower(), 0.0, 0.3])
        elif vtype == "bol":
            _send_osc("/atlas/rhythm/bol", [syllable, 0.8, 0])
        elif vtype == "mantra":
            amp = float(data.get("amp", 0.1))
            _send_osc("/atlas/bija", ["aum", "aum", "aum"])
        return app.response_class(
    ...
    @app.route("/sound/perform_mode", methods=["POST"])
    def _sound_perform_mode():
        nonlocal _perform_mode
        data = request.get_json(force=True)
        _perform_mode = data.get("active", False)
        if _perform_mode:
            # Drop to tanpura drone only — cosmos sequencer drives
            _send_osc("/atlas/field", [261.63, 72, 0, 0, 1.125, 0])
        else:
            # Resume full field
            _send_osc("/atlas/field", [261.63, 72, 0, 0, 1.125, 1])
        return app.response_class(
            json.dumps({"perform_mode": _perform_mode}, ensure_ascii=False),
            mimetype="application/json",
        )

    @app.route("/sound/perform_mode")
    ...
    @app.route("/sound/mode", methods=["GET", "POST"])
    def _sound_mode_endpoint():
        nonlocal _sound_mode
        if request.method == "POST":
            data = request.get_json(force=True)
            ...
            _sound_mode = new_mode
            try:
                from npu_engine.sound.sound_engine import derive_sound_spec
                from npu_engine.sound.osc_bridge import send_sound_spec
                spec = derive_sound_spec(field_state(), mode=_sound_mode)
                send_sound_spec(spec)
            except Exception as exc:
                return app.response_class(
                    json.dumps({"mode": _sound_mode, "osc_sent": False, "error": str(exc)}),
                    mimetype="application/json",
                )
            return app.response_class(
                json.dumps({"mode": _sound_mode, "osc_sent": True}),
                mimetype="application/json",
            )
        return app.response_class(
            json.dumps({"mode": _sound_mode}),
            mimetype="application/json",
        )

    @app.route("/sound/mix/state")
    ...
    def _sound_recommend():
        ...
        # Vara → graha (slug form for graph)
        _graha_slug = {
        ...
        try:
            ...
            # Dosha → therapeutic raga (reverse: find ragas targeting this dosha)
    ...
    @app.route("/plants/guild/<nakshatra_id>")
    ...
    @app.route("/plants/related/<path:entity_id>")
    ...
    @app.route("/plants/<path:plant_key>")
    ...
    def _brahmanda_state():
        ...
        try:
            ...
            _sound_spec = derive_sound_spec(_sound_fs, mode=_sound_mode)
    ...
    # ── Yantra field persistence (Supabase) ──────────────────────────
    ...
    @app.route("/yantra/deposit", methods=["POST"])
    def _yantra_deposit():
    ...
    @app.route("/system/audio/restore", methods=["POST"])
    ...
    def _render_bandhu():
        ...
        layers = request.args.get("layers", "skin")  # skin|dhatu|all|signature
        dhatu = request.args.get("dhatu", "")  # D001-D007 or empty
        zoom = float(request.args.get("zoom", 1.0))
        scale = raw_scale  # base_unit (2.8) is now baked into render_bandhu_svg
        ...
        return app.response_class(full_svg, mimetype="image/svg+xml")

    @app.route("/render/species/<species_id>")
    ...
    @app.route("/land/plot", methods=["POST"])
    def _land_plot():
        from npu_engine.field.land_engine import derive_land_layout
        data = request.get_json(force=True, silent=True) or {}
    ...
    @app.route("/land/mandala/recommend", methods=["POST"])
    ...
    _RESEARCH_ROOT = os.path.join(os.path.dirname(__file__), "research")

    _PROJECT_ROOT = os.path.dirname(__file__)
    _ALLOWED_ROOTS = {
        "wiki": os.path.join(_PROJECT_ROOT, "docs", "wiki"),
    }
    # allow datasets/<subdir> browsing
    _DS_ROOT = os.path.join(_PROJECT_ROOT, "datasets")

    @app.route("/research/files")
    def research_files():
    ...
    @app.route("/research/save", methods=["POST"])
    def research_save():
        data = request.get_json(force=True)
    ...
    @app.route("/research/datasets")
    def research_datasets():
        ds_root = os.path.join(os.path.dirname(__file__), "datasets")
        dirs = []
